 ...
 * 图片分享
 *
 * @param imgUrl                要分享的图片地址url
 * @param channels              分享的渠道：save-保存图片，weixin-微信，wxcircle-朋友圈，weibo-微博，qq-QQ，sms-短信，按顺序多个渠道用逗号","分隔，例如：save,weixin,qq，则弹出的分享面板依次为“保存、微信、QQ”
 * @param type                  分享的类型
 * @param shareTitle            分享的title
 */
function shareImageV2(imgUrl, channels, type, shareTitle) {
  var app = window.HMApplication;
  if (android) {
    app.shareImageV2(imgUrl, channels, type, shareTitle);
  } else {
    if (iosUI) {
      app.shareImageV2(imgUrl, channels, type, shareTitle);
    } else {
      if (iosWK) {
        ...
        WKWebView.shareImageV2.postMessage(params);
      }
    }
  }
}
 ...
 */
function shareLinkV2(title, desc, linkurl, channels, shareTitle) {
  var app = window.HMApplication;
  if (android) {
    app.shareLinkV2(title, desc, linkurl, channels, shareTitle);
  } else {
    if (iosUI) {
      app.shareLinkV2(title, desc, linkurl, channels, shareTitle);
    } else {
      if (iosWK) {
        ...
        WKWebView.shareLinkV2.postMessage(params);
      }
    }
  }
}
 ...
 * @param {String} channels        分享的渠道：save-保存图片，weixin-微信，wxcircle-朋友圈，weibo-微博，qq-QQ，sms-短信，按顺序多个渠道用逗号","分隔，例如：save,weixin,qq，则弹出的分享面板依次为“保存、微信、QQ”
 */
function shareTextV2(sharetext, channels, shareTitle) {
  var app = window.HMApplication;
  if (android) {
    app.shareTextV2(sharetext, channels, shareTitle);
  } else {
    if (iosUI) {
      app.shareTextV2(sharetext, channels, shareTitle);
    } else {
      if (iosWK) {
        ...
        WKWebView.shareTextV2.postMessage(params);
      }
    }
  }
}
 ...
 * @param {String} channels  分享的渠道：save-保存图片，weixin-微信，wxcircle-朋友圈，weibo-微博，qq-QQ，sms-短信，按顺序多个渠道用逗号","分隔，例如：save,weixin,qq，则弹出的分享面板依次为“保存、微信、QQ”
 */
function shareImageByBase64V2(img, channels, shareTitle) {
  var app = window.HMApplication;
  if (android) {
    app.shareImageByBase64V2(img, channels, shareTitle);
  } else {
    if (iosUI) {
      app.shareImageByBase64V2(img, channels, shareTitle);
    } else {
      if (iosWK) {
        ...
        WKWebView.shareImageByBase64V2.postMessage(params);
      }
    }
  }
}
 ...
 * @param dataList
 */
function notifyEvent(dataList) {
  var app = window.HMApplication;
  if (android) {
    app.notifyEvent(dataList);
  } else if (iosUI) {
    app.notifyEvent(dataList);
  } else if (iosWK) {
    var params = JSON.stringify({
    ...
    WKWebView.notifyEvent.postMessage(params);
  }
}
...
/**
 * 在当前的webview打开url（iOS特定方法）
 *
 * @param {String} linkUrl      网页地址
 */
function openUrlThroughWebViewInPage(linkUrl) {
  var app = window.HMApplication;
  app.openUrlThroughWebViewInPage(linkUrl);
}
